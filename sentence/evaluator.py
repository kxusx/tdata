...
def evalFunction():
	...
	for puzzle in puzzles:
		...
		error = 0
		isOneAway = False
		print("INvalid guesses: ", invalidGuesses)
		while strikes < 4 and len(correctGroups) < 4 and invalidGuesses < 7:
			...
			if any(np.array_equal(sortedGuess, np.sort(x)) for x in previousGuesses):
				print("You have already guessed this combination.")
